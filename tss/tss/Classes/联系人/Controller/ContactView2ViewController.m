...
@interface ContactView2ViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
...
@implementation ContactView2ViewController
...
#pragma mark - 公司组织
    else if(indexPath.section==1)
    {
        ...
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"联系人>成都绍平物流>%@",titleString]];
        [str addAttribute:NSForegroundColorAttributeName value:fontColor range:NSMakeRange(0,11)];
        [str addAttribute:NSForegroundColorAttributeName value:lvColor range:NSMakeRange(11,str.length-11)];
        titleLable2.attributedText = str;
        
        
        [titleLable2 makeConstraints:^(MASConstraintMaker *make)
         {
             make.left.equalTo(cell.contentView).with.offset(15);
             make.size.equalTo(CGSizeMake(290*APP_DELEGATE().autoSizeScaleX,46*APP_DELEGATE().autoSizeScaleY));
