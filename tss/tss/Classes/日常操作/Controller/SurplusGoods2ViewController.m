...
@interface SurplusGoods2ViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    
}

@property (nonatomic ,strong) UITableView *tableView;
@property (nonatomic ,strong) UILabel *wangdianLabel;
@property (nonatomic ,strong) UILabel *xianluLabel;
@end



@implementation SurplusGoods2ViewController
...
- (UILabel *)wangdianLabel
{
    if(!_wangdianLabel)
    {
        _wangdianLabel = [[UILabel alloc] init];
        _wangdianLabel.backgroundColor = [UIColor whiteColor];
        _wangdianLabel.numberOfLines = 0;
        _wangdianLabel.textAlignment = NSTextAlignmentRight;
        _wangdianLabel.font = viewFont3;
        _wangdianLabel.textColor = fontColor;
        _wangdianLabel.text = @"网点:小金牛";
    }
    
    return _wangdianLabel;
}
...
- (UILabel *)xianluLabel
{
    if(!_xianluLabel)
    {
        ...
        _xianluLabel.backgroundColor = [UIColor whiteColor];
        _xianluLabel.numberOfLines = 0;
        _xianluLabel.textAlignment = NSTextAlignmentLeft;
        _xianluLabel.font = viewFont3;
        _xianluLabel.textColor = fontColor;
        _xianluLabel.text = @"成都-泸州";
    }
...
- (void)initView
{
     ...
     [self.view addSubview:self.xianluLabel];
    
   
    
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0*APP_DELEGATE().autoSizeScaleX, 10*APP_DELEGATE().autoSizeScaleY, 320*APP_DELEGATE().autoSizeScaleX, 30*APP_DELEGATE().autoSizeScaleY)];
    view.backgroundColor = [UIColor whiteColor];
    
    [self.view addSubview:view];
    [view addSubview:self.wangdianLabel];
    [view addSubview:self.xianluLabel];
    
    self.xianluLabel.frame = CGRectMake(15*APP_DELEGATE().autoSizeScaleX, 0*APP_DELEGATE().autoSizeScaleY, 160*APP_DELEGATE().autoSizeScaleX, 30*APP_DELEGATE().autoSizeScaleY);
   
    self.wangdianLabel.frame = CGRectMake(145*APP_DELEGATE().autoSizeScaleX, 0*APP_DELEGATE().autoSizeScaleY, 160*APP_DELEGATE().autoSizeScaleX, 30*APP_DELEGATE().autoSizeScaleY);
