...
{
    if (str && [str isKindOfClass:[NSString class]] && [str length] > 0 && ![str isEqualToString:kString_Null] && ![str isEqualToString:kString_Null_IOS]) {
        return YES;
    }
    return NO;
}
...
{
    if ([sex isEqualToString:kParams_sex_female] || [sex isEqualToString:kParams_sex_female_0]) {
        return @"女士";
    }
    return @"先生";
}
...
- (BOOL)checkMobile
{
    ...
    NSString *regex = @"^((13[0-9])|(147)|(170)|(15[0-9])|(18[0-9]))\\d{8}$";
...
{
    if ([NSString isLegal:name]) {
        if ([name length] == 1) {
            if ([sex isEqualToString:kParams_sex_male] || [sex isEqualToString:kParams_sex_male_1]) {
                return [NSString stringWithFormat:@"%@先生", name];
            }
            else if ([sex isEqualToString:kParams_sex_female] || [sex isEqualToString:kParams_sex_female_0]) {
                return [NSString stringWithFormat:@"%@女士", name];
            }
            else {
                return name;
            }
...
{
    // 模板常量的数组
    NSArray *stringArray = [NSArray arrayWithObjects:@"${预订时间}",@"${预订人数}",@"${商户名称}",@"${姓名}",@"${性别}",@"${商家地址}",@"${商家电话}",@"${桌台}",@"${桌台类型}",nil];
    
    // 要替换的数据的数组
    NSMutableDictionary *dictM = [NSMutableDictionary dictionary];
    // 预订时间
    [NSString stringByRepleceWithDictionary:dictM withValue:orderTime WithKey:[stringArray objectAtIndex:0]];
    // 预订人数
    [NSString stringByRepleceWithDictionary:dictM withValue:personCount WithKey:[stringArray objectAtIndex:1]];
    // 商户名称
    [NSString stringByRepleceWithDictionary:dictM withValue:shopName WithKey:[stringArray objectAtIndex:2]];
    // 顾客姓名
    [NSString stringByRepleceWithDictionary:dictM withValue:customerName WithKey:[stringArray objectAtIndex:3]];
    // 性别
    [NSString stringByRepleceWithDictionary:dictM withValue:sex WithKey:[stringArray objectAtIndex:4]];
    // 商户地址
    [NSString stringByRepleceWithDictionary:dictM withValue:shopAddress WithKey:[stringArray objectAtIndex:5]];
    // 商户电话
    [NSString stringByRepleceWithDictionary:dictM withValue:shopMobile WithKey:[stringArray objectAtIndex:6]];
    // 预订桌台号码
    [NSString stringByRepleceWithDictionary:dictM withValue:deskNumber WithKey:[stringArray objectAtIndex:7]];
    // 预订桌类型
    [NSString stringByRepleceWithDictionary:dictM withValue:deskType WithKey:[stringArray objectAtIndex:8]];
    
    for (NSString *string in stringArray)
    {
        if ([NSString isLegal:string])
        {
            NSString *str = [dictM objectForKey:string];
            source = [source stringByReplacingOccurrencesOfString:string withString:str];
        }
    }
...
{
    // 模板常量的数组
    NSArray *stringArray = [NSArray arrayWithObjects:@"${商户名称}",@"${姓名}",@"${性别}",@"${商家地址}",@"${商家电话}",@"${排队号}",nil];
    
    NSInteger index = 0;
    // 要替换的数据的数组
    NSMutableDictionary *dictM = [NSMutableDictionary dictionary];
    // 商户名称
    [NSString stringByRepleceWithDictionary:dictM withValue:shopName WithKey:[stringArray objectAtIndex:index++]];
    // 顾客姓名
    [NSString stringByRepleceWithDictionary:dictM withValue:customerName WithKey:[stringArray objectAtIndex:index++]];
    // 性别
    [NSString stringByRepleceWithDictionary:dictM withValue:sex WithKey:[stringArray objectAtIndex:index++]];
    // 商户地址
    [NSString stringByRepleceWithDictionary:dictM withValue:shopAddress WithKey:[stringArray objectAtIndex:index++]];
    // 商户电话
    [NSString stringByRepleceWithDictionary:dictM withValue:shopMobile WithKey:[stringArray objectAtIndex:index++]];
    // 排队号
    [NSString stringByRepleceWithDictionary:dictM withValue:queueNumber WithKey:[stringArray objectAtIndex:index++]];
...
+ (void)stringByRepleceWithDictionary:(NSMutableDictionary *)dict withValue:(NSString *)value WithKey:(NSString *)key
{
...
//2014-06-30 17:27:45  20140630172745
